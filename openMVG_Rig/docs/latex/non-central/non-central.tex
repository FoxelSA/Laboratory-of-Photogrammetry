...
\begin{document}
   ...
   to solve equation \eqref{generalized epipolar}, which has 6 degrees of freedoms. 
   The most natural approach is to use the a singular-value decomposition to find $R$ and $\t$ in \eqref{generalized epipolar}, as
   ...
   explaination). This is major difference between non central cameras and central cameras, because in the central camera case, 
   the solution is unique up to a scale factor (see \cite{Hartley:2003:MVG:861369} for explanations). \paragraph{}
   
   Because the standard linear algorithm does not work in most cases, there is some others methods that have developped during
   ...
   be munch improved in order to gain accuracy for the whole global pipeline that we will present later on.
